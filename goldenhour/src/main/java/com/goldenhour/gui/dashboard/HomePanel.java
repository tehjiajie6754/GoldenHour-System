...
public class HomePanel extends JPanel {
    ...
    public void refreshData() {
        // Force re-fetch of sales data from database to ensure latest updates
    ...
    private void buildUI() {
        ...
        double todayProfit = DataLoad.allSales.stream().filter(s -> s.getDate().equals(todayStr))
        ...
        if (AuthService.getCurrentUser() != null) {
            isManager = "Manager".equalsIgnoreCase(AuthService.getCurrentUser().getRole());
        ...
        if (isManager) {
            ...
            gbc.gridwidth = 1;
            gbc.weightx = 0.25;
            mainGrid.add(createStatsWithChartCard("New Sales", "+" + todaySalesCount, "Count Today", COL_SUCCESS,
                    new MiniLineChart(COL_SUCCESS)), gbc);

            gbc.gridx = 3;
            gbc.gridwidth = 1;
            gbc.weightx = 0.25;
            mainGrid.add(createStatsWithChartCard("Active Staff", String.valueOf(staffCount), "Employees", COL_WARNING,
                    new MiniBarChart(COL_WARNING)), gbc);
            ...
            gbc.weighty = 0.25;
            mainGrid.add(
                    createStatCard("Profit (Today)", String.format("RM %.2f", todayProfit), "+12% ↗", COL_INFO, "💰"),
                    gbc);

            gbc.gridx = 3;
            gbc.gridy = 2;
            mainGrid.add(
                    createStatCard("Total Sales", String.format("RM %.2f", totalSalesAmt), "+28% ↗", COL_PRIMARY, "💼"),
                    gbc);
            ...
            gbc.gridheight = 1;
            gbc.weighty = 0.3;
            mainGrid.add(new TopProductPanel(), gbc);
            ...
            // 1. Employee Specific Data Logic
            final String finalEmpId = (currentUserId != null) ? currentUserId : "";

            // Count Today: New sales added by specific employee today
            long empTodaySalesCount = DataLoad.allSales.stream()
                    .filter(s -> s.getDate().equals(todayStr))
                    .filter(s -> s.getEmployeeId() != null && s.getEmployeeId().equals(finalEmpId))
                    .count();

            // Total Sales Today: Replace Active Staff
            double empTodaySalesAmt = DataLoad.allSales.stream()
                    .filter(s -> s.getDate().equals(todayStr))
                    .filter(s -> s.getEmployeeId() != null && s.getEmployeeId().equals(finalEmpId))
                    .mapToDouble(Sales::getSubtotal).sum();

            // Total Sales This Month: Replace Profit (Today)
            String currentMonth = todayStr.substring(0, 7); // yyyy-MM
            double empMonthSalesAmt = DataLoad.allSales.stream()
                    .filter(s -> s.getDate().startsWith(currentMonth))
                    .filter(s -> s.getEmployeeId() != null && s.getEmployeeId().equals(finalEmpId))
                    .mapToDouble(Sales::getSubtotal).sum();

            // KPI Goal: Fixed RM 8000
            double kpiGoal = 8000.0;
            double progress = (empMonthSalesAmt / kpiGoal) * 100;
            if (progress > 100)
                progress = 100;
            String kpiProgressStr = String.format("%.0f%% Achieved", progress);
            ...
            // 2) Count Today (Employee Specific)
            gbc.gridx = 2;
            gbc.gridwidth = 1;
            gbc.weightx = 0.25;
            mainGrid.add(createStatsWithChartCard("Your Sales", "+" + empTodaySalesCount, "Count Today", COL_SUCCESS,
                    new MiniLineChart(COL_SUCCESS)), gbc);

            // 3) Active Staff -> Total Sales of Employee Today
            gbc.gridx = 3;
            gbc.gridwidth = 1;
            gbc.weightx = 0.25;
            mainGrid.add(createStatsWithChartCard("Today's Volume", String.format("RM %.0f", empTodaySalesAmt),
                    "Sales Amount", COL_WARNING, new MiniBarChart(COL_WARNING)), gbc);

            // ROW 2
            // 6) Total Income -> Top 5 Most Sold Products
            gbc.gridx = 0;
            gbc.gridy = 1;
            gbc.gridwidth = 3;
            gbc.gridheight = 2;
            gbc.weightx = 0.75;
            gbc.weighty = 0.5;
            mainGrid.add(new TopProductPanel(), gbc); // Reused existing panel

            // 4) Profit (Today) -> Total Sales of Employee This Month
            gbc.gridx = 3;
            gbc.gridy = 1;
            gbc.gridwidth = 1;
            gbc.gridheight = 1;
            gbc.weightx = 0.25;
            gbc.weighty = 0.25;
            mainGrid.add(createStatCard("Month Sales", String.format("RM %.2f", empMonthSalesAmt), "Personal", COL_INFO,
                    "💰"), gbc);

            // 5) Total Sales -> Sales KPI (RM 8000)
            gbc.gridx = 3;
            gbc.gridy = 2;
            mainGrid.add(createStatCard("Monthly KPI", "RM 8,000", kpiProgressStr, COL_PRIMARY, "🎯"), gbc);

            // 7) Delete Top 5 most sold product (Row 3 col 0) and Recent transaction (Row 3
            // col 1-3)
            // Implicitly handled by NOT adding them to mainGrid here.
        }
    }
    ...
    private JPanel createTotalIncomeCard() {
        ...
        title.setForeground(COL_HEAD);
        JLabel sub = new JLabel("Report overview");
        sub.setFont(new Font("SansSerif", Font.PLAIN, 12));
        sub.setForeground(COL_TEXT);
        titleBox.add(title);
        titleBox.add(sub);
        ...
        filterBox.addActionListener(
                e -> graph.setYearAndFilter((String) yearBox.getSelectedItem(), (String) filterBox.getSelectedItem()));
    ...
    // (I will condense these for brevity, they are identical to the previous
    // working version)
    ...
    private JPanel createWelcomeCard() {
        ...
        textPanel.setBorder(new EmptyBorder(25, 25, 25, 0));
        ...
        name.setForeground(COL_HEAD);
        JLabel sub = new JLabel("Here's what's happening today.");
        ...
        iconLabel.setFont(new Font("SansSerif", Font.PLAIN, 70));
    ...
    private JPanel createStatsWithChartCard(String title, String value, String sub, Color color, JPanel chart) {
        ...
        t.setForeground(COL_HEAD);
        JLabel s = new JLabel(title);
        ...
        subL.setForeground(Color.LIGHT_GRAY);
        left.add(t);
        left.add(s);
        left.add(subL);
        card.add(left, BorderLayout.CENTER);
        card.add(chart, BorderLayout.EAST);
        return card;
    }
    ...
    private JPanel createStatCard(String title, String value, String change, Color iconBg, String icon) {
        ...
        il.setFont(new Font("SansSerif", Font.PLAIN, 20));
        iconBox.add(il);
    ...
    private JPanel createTransactionListCard() {
        ...
        card.setLayout(new BorderLayout());
        card.setBorder(new EmptyBorder(20, 20, 20, 20));
        JLabel t = new JLabel("Recent Transactions");
        ...
        List<Sales> recent = DataLoad.allSales.stream().sorted((s1, s2) -> s2.getDate().compareTo(s1.getDate()))
        ...
        for (Sales s : recent) {
            ...
            list.add(createTransactionItem(method, s.getDate(), "+RM " + String.format("%.2f", s.getSubtotal()), iconBg,
                    iconFg));
    ...
    private JPanel createTransactionItem(String title, String sub, String amt, Color bg, Color fg) {
        ...
        p.setOpaque(false);
        JPanel icon = new Card(bg);
        icon.setPreferredSize(new Dimension(40, 40));
        JLabel l = new JLabel(title.substring(0, 1));
        l.setForeground(fg);
    ...
    private static class MiniLineChart extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            int[] vals = { 20, 10, 30, 25, 40, 30, 50 };
            Path2D.Float p = new Path2D.Float();
            int step = w / (vals.length - 1);
            p.moveTo(0, h - vals[0]);
            for (int i = 1; i < vals.length; i++)
                p.lineTo(i * step, h - vals[i]);
            g2.setColor(color);
    ...
    private static class MiniBarChart extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            int[] vals = { 30, 50, 20, 40, 35 };
            g2.setColor(color);
            for (int i = 0; i < bars; i++) {
                int bh = vals[i];
                g2.fillRoundRect(i * (bw + gap), h - bh, bw, bh, 4, 4);
            }
        }
    }
    ...
    private class SmoothGraphPanel extends JPanel {
        ...
        private void refreshData() {
            synchronized (dataLock) {
                ...
                if (DataLoad.allSales == null)
                    DataLoad.allSales = new ArrayList<>();
                ...
                if (currentFilter.equals("Year")) {
                    for (int i = 1; i <= 12; i++)
                        aggData.put(String.valueOf(i), 0.0);
                ...
                // =============================================================
                    ...
                    for (Sales s : DataLoad.allSales) {
                        try {
                            LocalDate d = LocalDate.parse(s.getDate());
                            // Check if date falls within this week range (Inclusive) and matches target
                            // year
                ...
                // =============================================================
                    ...
                    LocalDate now = LocalDate.now();
                    for (int i = 0; i < 24; i++)
                        aggData.put(String.format("%02d", i), 0.0);

                    for (Sales s : DataLoad.allSales) {
                        if (s.getDate().equals(now.toString())
                                && LocalDate.parse(s.getDate()).getYear() == targetYear) {
                            try {
        ...
        private void findHoveredPoint() {
            synchronized (dataLock) {
                ...
                if (mousePt == null || points.isEmpty())
                    return;
                if (mousePt.x < LEFT_MARGIN)
                    return;
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (!renderPoints.isEmpty()) {
                ...
                for (int i = 0; i < renderPoints.size(); i++) {
                    ...
                    if (px >= LEFT_MARGIN - 20 && px <= w + 20) {
                        if (zoom < 1.5 && renderPoints.size() > 15 && i % 2 != 0)
                            continue;
            ...
            if (renderHoverPoint != null) {
                ...
                int[] polyY = drawBelow ? new int[] { hy + 12, hy + 20, hy + 20 }
                        : new int[] { hy - 12, hy - 20, hy - 20 };
